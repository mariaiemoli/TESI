...
{
	const size_type nbDof =  fractures-> getMeshFEM().nb_basic_dof();
	...
	node[ 0 ] = fractures-> getMeshFEM().point_of_basic_dof( 0 )[ 0 ];
	node[ 1 ] = fractures-> getLevelSet()->getData()->y_map( tmp0 );
		
	PointData temp = M_intersection.getPointIntersection();
	if( fractures->getData().getInt() != 1000000 )
	{
		DOF = fractures->getData().getInt();
	}
	else
	{
